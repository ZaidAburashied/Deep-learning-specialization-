...
shape_X = X.shape
shape_Y = Y.shape
m = X.shape[1]  # Assuming the number of examples is along the second axis
# YOUR CODE ENDS HERE
...
def layer_sizes(X, Y):
    ...
    n_h = 4            # Hidden layer size (hard-coded to 4 for this exercise)
    n_y = Y.shape[0]  # Number of classes in the output layer
...
def forward_propagation(X, parameters):
    ...
    Z1 = np.dot(W1, X) + b1
    A1 = np.tanh(Z1)  # Use tanh activation for the first layer
    Z2 = np.dot(W2, A1) + b2
    A2 = 1 / (1 + np.exp(-Z2))  # Use sigmoid activation for the second layer
